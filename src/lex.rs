...
use crate::bytecode::ByteCode::BitNot;
use crate::lex::Token::{Break, Else, Eos, Len, Less, Repeat, Sub, While};
...
impl<R: Read> Lex<R> {
    ...
    fn read_decimal(&mut self,ahead:char) -> Token{
        ...
        if is_float{
            Token::ToFloat(buf.parse().unwrap())
        }else {
            Token::ToInteger(buf.parse().unwrap())
        }
    }
